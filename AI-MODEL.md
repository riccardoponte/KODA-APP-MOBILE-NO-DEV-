...
The reviewed bilingual glossary covers 22 concepts: Projects, custom assistants, Gems, Connectors, Plugins, Skills, tool use/function calling, MCP, AI agents, Computer Use, memory, Artifacts/Canvas, Deep Research, RAG, embeddings, fine-tuning, multimodal AI, reasoning models, context windows, tokens, guardrails, and grounding/hallucinations. These requests do not invoke SmolLM2. The router returns a stable `ai-concept-<id>` response type and related-concept quick replies.

The React chat maps every response type to a concept-specific diagram assembled from device, hub, loop, pipeline, convergence, vector-cluster, and context-window visual structures. The diagrams use CSS animation, retain a textual accessible label, adapt to the rendered message width, and stop moving when `prefers-reduced-motion` is enabled. The full written explanation remains visible below each diagram.

## Guided Interaction

Greetings and short or incomplete messages open a deterministic menu with three paths: find an AI tool, compare two AI tools, or explore AI concepts. Each path is an explicit JavaScript state machine and exposes optional quick-reply buttons. The concept selector pages all 22 reviewed entries six at a time and accepts its buttons, typed labels, or page-relative numbers. Comparison suggestions are read from the full live catalog, ordered with featured records first and then alphabetically, and paged four at a time through `Show more`/`Mostra altri`. Suggestions that represent catalog tools include their live catalog logo, with initials as a loading or error fallback. Users can still type a complete freeform question about AI or the catalog from the main menu.
